...
def SIFT():
    ...
    for l in range(0,truth_array_len):
    	len_h = len(truth_array[l])
    	wid_h = len(truth_array[l][0])

    	img = np.array(truth_array[l])
    	...
    	kp1_temp, des1_temp = sift.detectAndCompute(img_1, None)
    	kp1_1.append(kp1_temp)
    	des1_1.append(des1_temp)
    	
    	kp1_temp, des1_temp = sift.detectAndCompute(img_2, None)
    	kp1_2.append(kp1_temp)
    	des1_2.append(kp1_temp)

    	kp1_temp, des1_temp = sift.detectAndCompute(img_3, None)
    	kp1_3.append(kp1_temp)
    	des1_3.append(des1_temp)

    	kp1_temp, des1_temp = sift.detectAndCompute(img_4, None)
    	kp1_4.append(kp1_temp)
    	des1_4.append(des1_temp)
    #old code below
    ...
    for i in range(45,70) :
        ...
        # kp2_1, des2_1 = sift.detectAndCompute(img_array[i],None)
        ...
        for l in range(0,truth_array_len) :
        	...
        	if len(kp1_1[l])!=0 and len(kp2_1)!=0:
        		matches_1 = flann.knnMatch(des1_1[l],des2_1,k=2)
        		for q,(m,n) in enumerate(matches_1):
        			if m.distance < 0.6*n.distance:
                            good.append([m])
                            good_count += 1

            if len(kp1_2[l])!=0 and len(kp2_2)!=0:
        		matches_2 = flann.knnMatch(des1_2[l],des2_2,k=2)
        		for q,(m,n) in enumerate(matches_2):
        			if m.distance < 0.6*n.distance:
                            good.append([m])
                            good_count += 1

            if len(kp1_3[l])!=0 and len(kp2_3)!=0:
        		matches_3 = flann.knnMatch(des1_3[l],des2_3,k=2)
        		for q,(m,n) in enumerate(matches_3):
        			if m.distance < 0.6*n.distance:
                            good.append([m])
                            good_count += 1

            if len(kp1_4[l])!=0 and len(kp2_4)!=0:
        		matches_4 = flann.knnMatch(des1_4[l],des2_4 ,k=2)
        		for q,(m,n) in enumerate(matches_4):
        			if m.distance < 0.6*n.distance:
                            good.append([m])
                            good_count += 1
